...
var lightDirU, lEU, reflectiveU, eyePosU;
var toggle = 1;
var r = 5;
var g_tex_ready = 0;
...
*/
  ...
  sampler = gl.getUniformLocation(program, 'sphereMap');
  gl.uniform1i(sampler, 1);
  gl.drawArrays(gl.TRIANGLE_FAN, 0, index);//(gl.LINE_STRIP, n, gl.UNSIGNED_BYTE, 0);
...
function init_textures(gl)
{
  ...
  /*
  var cubemap = ['textures/cm_right.png', // POSITIVE_X
...
function init()
{
  ...
  pointsArray = [
    vec4(-1000.0, -1.0, -2000.0, 1.0),
    vec4(1000.0, -1.0, -2000.0, 1.0),
    vec4(1000.0, -1.0, 2000.0, 1.0),
    vec4(-1000.0, -1.0, 2000.0, 1.0),
  ];
  normalsArray = [
    vec4(-100.0, -1.0, -100.0, 1.0),
    vec4(100.0, -1.0, -100.0, 1.0),
    vec4(100.0, -1.0, 100.0, 1.0),
    vec4(-100.0, -1.0, 100.0, 1.0),
  ];
  ...
  gl.bufferData(gl.ARRAY_BUFFER, flatten(tex_coords), gl.DYNAMIC_DRAW);
